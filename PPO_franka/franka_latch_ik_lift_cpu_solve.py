...
franka_asset = gym.load_asset(
    sim, asset_root, "urdf/franka_description/robots/franka_panda.urdf", franka_options
)
...
latch_asset = gym.load_asset(sim, asset_root, "urdf/think_shell_slider.urdf", latch_options)
...
def enforce_latch_constraints():
    ...
    gym.set_dof_state_tensor_indexed(
        ...
        gymtorch.unwrap_tensor(latch_actor_indices),
        latch_actor_indices.numel(),
    )
...
def observe():
    ...
    latch_is_pushed = latch_horizontal_force > 0.10
    left_contact = (
        (torch.norm(contact_forces[left_finger_indices], dim=-1) > 0.10)
        & (left_distance < 0.075) & latch_is_pushed
    ).float()
    right_contact = (
        (torch.norm(contact_forces[right_finger_indices], dim=-1) > 0.10)
        & (right_distance < 0.075) & latch_is_pushed
    ).float()
...
def reset(indices):
    ...
    actors = torch.cat((franka_actor_indices[indices], latch_actor_indices[indices])).contiguous()
    gym.set_dof_state_tensor_indexed(
        sim,
        gymtorch.unwrap_tensor(dof_states),
        gymtorch.unwrap_tensor(actors),
        actors.numel(),
    )
    ...
    gym.set_actor_root_state_tensor_indexed(
        ...
        gymtorch.unwrap_tensor(latch_actor_indices[indices].contiguous()),
        indices.numel(),
    )
...
while viewer is None or not gym.query_viewer_has_closed(viewer):
    ...
    if not args.eval and args.approach_assist > 0:
    ...
    latch_is_pushed = latch_horizontal_force > 0.10
    left_contact = (
        (left_force > 0.10) & (left_finger_distance < 0.075) & latch_is_pushed
    )
    right_contact = (
        (right_force > 0.10) & (right_finger_distance < 0.075) & latch_is_pushed
    )
    ...
    closed_around_latch = (
        (finger_width < 0.075)
        & (left_finger_distance < 0.090)
        & (right_finger_distance < 0.090)
    )
    newly_grasped = bilateral_contact & (contact_streak >= 2) & closed_around_latch
